 ...
 * @param roles Roles
 */
export function filterAuthRoutesByRoles(routes: AppRouteRecordRaw[], roles: string[]) {
 ...
 * @param roles Roles
 */
function filterAuthRouteByRoles(route: AppRouteRecordRaw, roles: string[]): AppRouteRecordRaw[] {
  ...
  if (filterRoute.children?.length) {
    filterRoute.children = filterRoute.children.flatMap((item: AppRouteRecordRaw) =>
 ...
 * @param route route
 */
function sortRouteByOrder(route: AppRouteRecordRaw) {
  if (route.children?.length) {
    route.children.sort(
      (next: AppRouteRecordRaw, prev: AppRouteRecordRaw) =>
 ...
 * @param routes routes
 */
export function sortRoutesByOrder(routes: AppRouteRecordRaw[]) {
 ...
 * @param route
 */
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | AppRouteRecordRaw) {
  const { name, path } = route
  const { title, icon, iconSize } = route.meta ?? {}

  const label = title || (name as string)
 ...
 * @param routes
 */
export function isRouteExistByRouteName(routeName: string, routes: AppRouteRecordRaw[]) {
 ...
 * @param routeName
 */
function recursiveGetIsRouteExistByRouteName(route: AppRouteRecordRaw, routeName: string) {
  ...
  if (route.children && route.children.length) {
    isExist = route.children.some((item: AppRouteRecordRaw) => recursiveGetIsRouteExistByRouteName(item, routeName))
  }
